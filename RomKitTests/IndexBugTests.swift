...
struct IndexBugTests {
    ...
    @Test("Test ZIP indexing extracts all entries", .disabled("FastZIPHandler.create not fully implemented"))
    ...
    @Test("Test multiple ZIPs in directory", .disabled("FastZIPHandler.create not fully implemented"))
    func testMultipleZIPs() async throws {
        ...
        for index in 1...3 {
            let zipPath = testDir.appendingPathComponent("game\(index).zip")
            let entries: [(name: String, data: Data)] = [
                ("rom_a.bin", Data(repeating: UInt8(index), count: 1024)),
                ("rom_b.bin", Data(repeating: UInt8(index + 10), count: 1024))
            ]
            try handler.create(at: zipPath, with: entries)
        }
