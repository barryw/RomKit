...
struct SyntheticROMTests {
    ...
    @Test func testStorageEfficiency() async throws {
        ...
        var results: [(style: String, totalSize: Int, fileCount: Int)] = []
        ...
        for style in [RebuildOptions.Style.split, .merged, .nonMerged] {
            ...
            results.append((String(describing: style), totalSize, fileCount))
        }
        ...
        let _ = results.first { $0.style == "split" }?.totalSize ?? 0
        let _ = results.first { $0.style == "nonMerged" }?.totalSize ?? 0
